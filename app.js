...
(() => {
  ...
  const defaultState = () => ({
    profile: null,
    goals: { cal: 2200, protein: 150, carbs: 250, fat: 70, goalType: 'maintain', goalIntensity: 'moderate' },
    days: {}
  });
  ...
  const yesterdayKey = () => { const d = new Date(); d.setDate(d.getDate() - 1); return localKey(d); };
  const isPastDay = () => currentDate < yesterdayKey();
  const uid = () => Math.random().toString(36).slice(2, 10) + Date.now().toString(36);

  const GOAL_OFFSETS = {
    cut:      { light: -150, moderate: -300, aggressive: -500 },
    maintain: { light: 0,    moderate: 0,    aggressive: 0 },
    bulk:     { light: 150,  moderate: 300,  aggressive: 500 }
  };
  const ensureDay = (key) => {
  ...
  // ═══════════════════════════════════════════════════════════
  const renderBalancePanel = (t) => {
    const panel = $('#balancePanel');
    const tdee  = calcTDEE(state.profile);
    if (!tdee) { panel.style.display = 'none'; return; }
    panel.style.display = '';

    const delta = Math.round(t.net - tdee);
    const absD  = Math.abs(delta);
    const type  = delta < -50 ? 'deficit' : delta > 50 ? 'surplus' : 'balanced';

    const sign = delta < -50 ? '−' : delta > 50 ? '+' : '';
    $('#balanceNumber').textContent = sign + fmtNum(absD);
    $('#balanceNumber').className   = `balance-number balance-number--${type}`;
    $('#balanceTag').textContent    = type.toUpperCase();
    $('#balanceTag').className      = `balance-tag balance-tag--${type}`;
    $('#balanceVsTDEE').textContent = `vs ${fmtNum(tdee)} TDEE`;

    const weeklyKg = (absD * 7 / 7700).toFixed(2);
    $('#balanceProjected').textContent = type === 'balanced'
      ? '≈ 0 KG / WK'
      : `≈ ${delta < 0 ? '−' : '+'}${weeklyKg} KG / WK`;

    const goalType      = state.goals.goalType      || 'maintain';
    const goalIntensity = state.goals.goalIntensity || 'moderate';
    const goalTag       = $('#balanceGoalTag');
    if (goalType === 'maintain') {
      goalTag.textContent = 'GOAL · MAINTENANCE';
    } else {
      const offset = GOAL_OFFSETS[goalType][goalIntensity];
      goalTag.textContent = `${goalIntensity.toUpperCase()} ${goalType.toUpperCase()} · TARGET ${offset > 0 ? '+' : ''}${offset} KCAL`;
    }
  };
  ...
  let weekChartMode = 'cal';

  const renderWeekChart = () => {
    if (!weekChart) return;
    const c = themeColors();
    const keys = last7Keys();
    const labels = keys.map(k => new Date(k + 'T00:00:00').toLocaleDateString(undefined, { weekday: 'short' }).toUpperCase().slice(0, 3));
    weekChart.data.labels = labels;

    if (weekChartMode === 'weight') {
      const units = state.profile?.units || 'metric';
      const suffix = units === 'imperial' ? ' lb' : ' kg';
      weekChart.data.datasets = [{
        label: 'Weight',
        data: keys.map(k => { const w = state.days[k]?.weight; return w != null ? +((units === 'imperial' ? w * 2.20462 : w).toFixed(1)) : null; }),
        type: 'line',
        borderColor: c.amber, backgroundColor: 'rgba(255,157,42,.12)',
        tension: .35, borderWidth: 2.5, pointRadius: 4,
        pointBackgroundColor: c.amber, pointBorderColor: c.bg2, pointBorderWidth: 1.5,
        fill: true, spanGaps: true
      }];
      weekChart.options.scales.y.beginAtZero = false;
      weekChart.options.scales.y.ticks.callback = v => v + suffix;
      weekChart.options.plugins.tooltip.callbacks.label = ctx => ` ${ctx.parsed.y}${suffix}`;
      $('#weekLegend').innerHTML = `<span><em class="dot dot-amber"></em>${units === 'imperial' ? 'LBS' : 'KG'}</span>`;
    } else {
      const intake = [], burned = [], net = [];
      keys.forEach(k => { const t = totalsFor(k); intake.push(Math.round(t.cal)); burned.push(Math.round(t.burned)); net.push(Math.round(t.net)); });
      weekChart.data.datasets = [
        { label: 'Intake', data: intake, backgroundColor: c.green, borderRadius: 5, barPercentage: .7, categoryPercentage: .55 },
        { label: 'Burned', data: burned, backgroundColor: c.coral, borderRadius: 5, barPercentage: .7, categoryPercentage: .55 },
        { label: 'Net',    data: net, type: 'line', borderColor: c.amber, backgroundColor: 'rgba(255,157,42,.12)', tension: .35, borderWidth: 2.5, pointRadius: 3, pointBackgroundColor: c.amber, pointBorderColor: c.bg2, pointBorderWidth: 1.5, fill: true, order: 0 }
      ];
      weekChart.options.scales.y.beginAtZero = true;
      weekChart.options.scales.y.ticks.callback = undefined;
      weekChart.options.plugins.tooltip.callbacks.label = ctx => ` ${ctx.dataset.label}: ${Math.round(ctx.parsed.y)} kcal`;
      $('#weekLegend').innerHTML = '<span><em class="dot dot-green"></em>IN</span><span><em class="dot dot-coral"></em>OUT</span><span><em class="dot dot-amber"></em>NET</span>';
    }
    weekChart.update();
  };
  ...
  const renderWeightPanel = () => {
    const units   = state.profile?.units || 'metric';
    const toDisp  = kg => units === 'imperial' ? +(kg * 2.20462).toFixed(1) : +kg.toFixed(1);
    const todayW  = state.days[currentDate]?.weight;
    const yestW   = state.days[yesterdayKey()]?.weight;
    const profileW = state.profile?.weightKg ?? null;
    const defW    = todayW ?? yestW ?? profileW;
    const input   = $('#weightInput');
    $('#weightUnitLabel').textContent = units === 'imperial' ? 'LBS' : 'KG';
    input.value = defW != null ? toDisp(defW) : '';
    input.classList.toggle('weight-input--saved', todayW != null);
  };
  ...
  registerSeg('goalsTypeSeg', v => {
    $('#goalsIntensityField').style.display = v === 'maintain' ? 'none' : '';
    updateApplyGoalHint();
  });
  registerSeg('goalsIntensitySeg', () => updateApplyGoalHint());
  ...
  const doLogin = async () => {
    ...
    try {
      ...
      if (pw !== atob(user.password).trim()) throw new Error('Incorrect password.');
      if (!user.jsonbin_key || !user.jsonbin_bin) throw new Error('User config incomplete — contact support.');
      const session = { id: user.id, jsonbin_key: atob(user.jsonbin_key).trim(), jsonbin_bin: atob(user.jsonbin_bin).trim() };
  ...
  const navTo = (viewId) => {
    ...
    if (viewId === 'settingsCoachKey') populateCoachKeyView();
    if (viewId === 'settingsFormulas') populateFormulasView();
  };
  ...
  const populateSettingsRoot = () => {
    // Update AI coach key sub-label on root
    const rootSub = $('#coachKeyRootSub');
    if (rootSub) {
      let k = ''; try { k = localStorage.getItem(COACH_KEY_LS) || ''; } catch {}
      rootSub.textContent = k ? 'Key saved · tap to update' : 'Groq API key for the coach tab';
    }
  ...
  // ── Populate My Goals ────────────────────────────────────────
  const updateApplyGoalHint = () => {
    const goalType      = $$('#goalsTypeSeg .seg-btn').find(b => b.classList.contains('active'))?.dataset.val || 'maintain';
    const goalIntensity = $$('#goalsIntensitySeg .seg-btn').find(b => b.classList.contains('active'))?.dataset.val || 'moderate';
    const tdee = calcTDEE(state.profile);
    if (!tdee) { $('#applyGoalHint').textContent = 'Complete MY INFO first to use this'; return; }
    const offset = GOAL_OFFSETS[goalType][goalIntensity];
    const target = tdee + offset;
    const sign   = offset > 0 ? `+${offset}` : offset < 0 ? `${offset}` : '±0';
    $('#applyGoalHint').textContent = goalType === 'maintain'
      ? `TDEE ${fmtNum(tdee)} kcal — maintenance`
      : `TDEE ${fmtNum(tdee)} ${sign} = ${fmtNum(target)} kcal`;
  };
  ...
  const populateGoalsView = () => {
    ...
    const goalType      = state.goals.goalType      || 'maintain';
    const goalIntensity = state.goals.goalIntensity || 'moderate';
    $$('#goalsTypeSeg .seg-btn').forEach(b => b.classList.toggle('active', b.dataset.val === goalType));
    $$('#goalsIntensitySeg .seg-btn').forEach(b => b.classList.toggle('active', b.dataset.val === goalIntensity));
    $('#goalsIntensityField').style.display = goalType === 'maintain' ? 'none' : '';
    updateApplyGoalHint();
    // Sleep slider
  ...
  $('#goalsForm').addEventListener('submit', async (e) => {
    ...
    state.goals = {
      ...
      goalType:      $$('#goalsTypeSeg .seg-btn').find(b => b.classList.contains('active'))?.dataset.val || 'maintain',
      goalIntensity: $$('#goalsIntensitySeg .seg-btn').find(b => b.classList.contains('active'))?.dataset.val || 'moderate'
    };
    if (state.profile) {
  ...
  $('#applyGoalPreset').addEventListener('click', () => {
    const tdee = calcTDEE(state.profile);
    if (!tdee) { toast('Complete MY INFO first', 'error'); return; }
    const goalType      = $$('#goalsTypeSeg .seg-btn').find(b => b.classList.contains('active'))?.dataset.val || 'maintain';
    const goalIntensity = $$('#goalsIntensitySeg .seg-btn').find(b => b.classList.contains('active'))?.dataset.val || 'moderate';
    const target = tdee + GOAL_OFFSETS[goalType][goalIntensity];
    $('#goalsForm').cal.value = target;
    $('#goalsCalDisplay').textContent = fmtNum(target);
    updateApplyGoalHint();
  });
  ...
  const COACH_KEY_LS = 'fuelgauge.groq_key';

  const populateCoachKeyView = () => {
    let existing = '';
    try { existing = localStorage.getItem(COACH_KEY_LS) || ''; } catch {}

    const badge     = $('#coachKeyBadge');
    const statusSub = $('#coachKeyStatusSub');
    const deleteBtn = $('#coachKeyDeleteBtn');

    if (existing) {
      badge.textContent = 'SET';
      badge.className   = 'storage-badge badge-opfs';
      statusSub.textContent = '••••' + existing.slice(-4);
      deleteBtn.style.display = '';
    } else {
      badge.textContent = 'NONE';
      badge.className   = 'storage-badge';
      statusSub.textContent = 'No key saved';
      deleteBtn.style.display = 'none';
    }

    // Update root list sub-label
    const rootSub = $('#coachKeyRootSub');
    if (rootSub) rootSub.textContent = existing ? 'Key saved · tap to update' : 'Groq API key for the coach tab';

    $('#coachKeyInput').value = '';

    $('#coachKeySaveBtn').onclick = () => {
      const val = $('#coachKeyInput').value.trim();
      if (!val) { toast('Paste your Groq API key first'); return; }
      pendingDelete = () => {
        try { localStorage.setItem(COACH_KEY_LS, val); } catch {}
        populateCoachKeyView();
        toast('API key saved');
      };
      $('#confirmTitle').textContent = 'Save API Key?';
      $('#confirmDesc').textContent  = 'Stored locally on this device only.';
      $('#confirmYes').textContent   = 'SAVE';
      openSheet('confirmSheet');
    };

    $('#coachKeyDeleteBtn').onclick = () => {
      pendingDelete = () => {
        try { localStorage.removeItem(COACH_KEY_LS); } catch {}
        populateCoachKeyView();
        toast('API key removed');
      };
      $('#confirmTitle').textContent = 'Remove API Key?';
      $('#confirmDesc').textContent  = 'The AI Coach will stop working until a new key is added.';
  ...
  const fHl = (v) => `<span class="formula-highlight">${v}</span>`;
  const noProfileHtml = '<div class="formula-note">Complete MY INFO first to see your values here.</div>';

  const formulaHtmlBMR = () => {
    const p = state.profile;
    if (!p) return noProfileHtml;
    const sStr = p.gender === 'male' ? '+5' : '−161';
    const bmr  = calcBMR(p);
    return `
      <div class="formula-section">
        <div class="formula-section-title">Mifflin-St Jeor Equation</div>
        <div class="formula-eq">BMR = (10 × weight) + (6.25 × height) − (5 × age) + s<br>s = +5 for male · −161 for female</div>
      </div>
      <div class="formula-section">
        <div class="formula-section-title">Your Values</div>
        <div class="formula-eq">(10 × ${fHl(p.weightKg + ' kg')}) + (6.25 × ${fHl(p.heightCm + ' cm')}) − (5 × ${fHl(p.age + ' yrs')}) ${sStr}<br>= ${Math.round(10 * p.weightKg)} + ${Math.round(6.25 * p.heightCm)} − ${5 * p.age} ${sStr}</div>
      </div>
      <div class="formula-section">
        <div class="formula-section-title">Result</div>
        <div class="formula-result-wrap"><span class="formula-result">${fmtNum(bmr)}</span><span class="formula-result-unit">kcal / day</span></div>
      </div>
      <div class="formula-note">Calories your body burns at complete rest. Published by Mifflin &amp; St Jeor (1990) — the most validated equation for most adults.</div>
    `;
  };

  const formulaHtmlTDEE = () => {
    const p = state.profile;
    if (!p) return noProfileHtml;
    const bmr  = calcBMR(p);
    const tdee = calcTDEE(p);
    const factorLabels = { '1.2': 'Sedentary', '1.375': 'Light', '1.55': 'Moderate', '1.725': 'Active', '1.9': 'Very Active' };
    const label = factorLabels[String(p.activityLevel)] || String(p.activityLevel);
    return `
      <div class="formula-section">
        <div class="formula-section-title">Formula</div>
        <div class="formula-eq">TDEE = BMR × Activity Factor</div>
      </div>
      <div class="formula-section">
        <div class="formula-section-title">Your Values</div>
        <div class="formula-eq">${fHl(fmtNum(bmr) + ' kcal BMR')} × ${fHl(p.activityLevel + ' (' + label + ')')}</div>
      </div>
      <div class="formula-section">
        <div class="formula-section-title">Result</div>
        <div class="formula-result-wrap"><span class="formula-result">${fmtNum(tdee)}</span><span class="formula-result-unit">kcal / day</span></div>
      </div>
      <div class="formula-note">Activity factors: Sedentary ×1.2 · Light ×1.375 · Moderate ×1.55 · Active ×1.725 · Very Active ×1.9<br>TDEE accounts for your daily movement on top of resting metabolism.</div>
    `;
  };

  const formulaHtmlBalance = () => {
    const p = state.profile;
    if (!p) return noProfileHtml;
    const t      = totalsFor(currentDate);
    const tdee   = calcTDEE(p);
    ...
    const type   = delta < -50 ? 'deficit' : delta > 50 ? 'surplus' : 'balanced';
    const weekKg = (absD * 7 / 7700).toFixed(2);
    return `
      <div class="formula-section">
        <div class="formula-section-title">Energy Balance</div>
        <div class="formula-eq">Balance = Net Calories − TDEE<br>= (Food In − Calories Burned) − TDEE</div>
      </div>
      <div class="formula-section">
        <div class="formula-section-title">Your Values Today</div>
        <div class="formula-eq">(${fHl(fmtNum(t.cal) + ' in')} − ${fHl(fmtNum(t.burned) + ' burned')}) − ${fHl(fmtNum(tdee) + ' TDEE')}<br>= ${fHl(sign + fmtNum(absD) + ' kcal')} · ${type}</div>
      </div>
      <div class="formula-section">
        <div class="formula-section-title">Weekly Projection</div>
        <div class="formula-eq">|Balance| × 7 ÷ 7,700 kcal per kg<br>${fHl(fmtNum(absD))} × 7 ÷ 7,700 = ${fHl(sign + weekKg + ' kg / wk')}</div>
      </div>
      <div class="formula-result-wrap"><span class="formula-result">${sign}${fmtNum(absD)}</span><span class="formula-result-unit">kcal balance</span></div>
      <div class="formula-note">~7,700 kcal ≈ 1 kg of body fat. A deficit loses weight; a surplus gains it. Assumes consistent daily intake.</div>
    `;
  };

  const formulaHtmlCalPerMin = () => {
    const wkg = state.profile?.weightKg || 70;
    return `
      <div class="formula-section">
        <div class="formula-section-title">MET Formula</div>
        <div class="formula-eq">Cal/min = (MET × weight × 3.5) ÷ 200</div>
      </div>
      <div class="formula-section">
        <div class="formula-section-title">Example at ${fHl(wkg + ' kg')}</div>
        <div class="formula-eq">Running (MET 9.8): (9.8 × ${wkg} × 3.5) ÷ 200 = ${fHl(((9.8 * wkg * 3.5) / 200).toFixed(1) + ' cal/min')}<br>Walking (MET 3.5): (3.5 × ${wkg} × 3.5) ÷ 200 = ${fHl(((3.5 * wkg * 3.5) / 200).toFixed(1) + ' cal/min')}</div>
      </div>
      <div class="formula-note">MET (Metabolic Equivalent of Task) = energy cost relative to rest. 1 MET ≈ 3.5 mL O₂/kg/min. Actual burn varies with fitness level and intensity.</div>
    `;
  };

  const formulaHtmlAutoMacros = () => {
    const cal  = parseInt($('#goalsForm')?.cal?.value, 10) || state.goals.cal;
    ...
    return `
      <div class="formula-section">
        <div class="formula-section-title">40 / 30 / 30 Split</div>
        <div class="formula-eq">Protein = (calories × 30%) ÷ 4 kcal/g<br>Carbs &nbsp; = (calories × 40%) ÷ 4 kcal/g<br>Fat &nbsp; &nbsp; = (calories × 30%) ÷ 9 kcal/g</div>
      </div>
      <div class="formula-section">
        <div class="formula-section-title">With ${fmtNum(cal)} kcal</div>
        <div class="formula-eq">Protein = (${fHl(fmtNum(cal))} × 30%) ÷ 4 = ${fHl(prot + ' g')}<br>Carbs &nbsp; = (${fHl(fmtNum(cal))} × 40%) ÷ 4 = ${fHl(carb + ' g')}<br>Fat &nbsp; &nbsp; = (${fHl(fmtNum(cal))} × 30%) ÷ 9 = ${fHl(fat + ' g')}</div>
      </div>
      <div class="formula-note">Protein and carbs = 4 kcal/g · Fat = 9 kcal/g. The 40/30/30 split balances energy, muscle maintenance, and satiety.</div>
    `;
  };

  const formulaHtmlGoalPreset = () => {
    const p = state.profile;
    if (!p) return noProfileHtml;
    const tdee         = calcTDEE(p);
    const goalType      = state.goals.goalType     || 'maintain';
    ...
    const offset       = GOAL_OFFSETS[goalType][goalIntensity];
    const target       = tdee + offset;
    const sign         = offset > 0 ? `+${offset}` : offset < 0 ? String(offset) : '±0';
    const offsetLabel  = goalType === 'maintain' ? 'maintenance — no offset' : `${goalIntensity} ${goalType} · ${sign} kcal`;
    return `
      <div class="formula-section">
        <div class="formula-section-title">Formula</div>
        <div class="formula-eq">Target Calories = TDEE + Goal Offset</div>
      </div>
      <div class="formula-section">
        <div class="formula-section-title">Your Values</div>
        <div class="formula-eq">${fHl(fmtNum(tdee) + ' TDEE')} + ${fHl(sign + ' (' + offsetLabel + ')')}</div>
      </div>
      <div class="formula-section">
        <div class="formula-section-title">Result</div>
        <div class="formula-result-wrap"><span class="formula-result">${fmtNum(target)}</span><span class="formula-result-unit">kcal / day</span></div>
      </div>
      <div class="formula-note">Cut: −150 / −300 / −500 kcal (light / moderate / aggressive)<br>Maintain: no offset<br>Bulk: +150 / +300 / +500 kcal (light / moderate / aggressive)</div>
    `;
  };

  const populateFormulasView = () => {
    const sections = [
      { title: 'BMR · MIFFLIN-ST JEOR',         html: formulaHtmlBMR() },
      { title: 'TDEE · TOTAL DAILY EXPENDITURE', html: formulaHtmlTDEE() },
      { title: 'ENERGY BALANCE',                 html: formulaHtmlBalance() },
      { title: 'CAL / MIN · MET FORMULA',        html: formulaHtmlCalPerMin() },
      { title: 'AUTO-DISTRIBUTE MACROS',         html: formulaHtmlAutoMacros() },
      { title: 'SET CALORIES FROM GOAL',         html: formulaHtmlGoalPreset() },
    ];
    $('#formulasViewBody').innerHTML = sections.map(s => `
      <div class="formulas-section-wrap">
        <div class="formulas-section-heading">${s.title}</div>
        ${s.html}
      </div>
    `).join('');
  };
  ...
  // Week chart mode toggle
  $$('.week-mode-btn').forEach(btn => btn.addEventListener('click', () => {
    weekChartMode = btn.dataset.mode;
    $$('.week-mode-btn').forEach(b => b.classList.toggle('week-mode-btn--active', b === btn));
    renderWeekChart();
  }));
  ...
  $('#weightSaveBtn').addEventListener('click', async () => {
    ...
    if (!val || val <= 0) return;
    const units = state.profile?.units || 'metric';
    const kg = units === 'imperial' ? Math.round((val / 2.20462) * 100) / 100 : Math.round(val * 10) / 10;
    ensureDay(currentDate).weight = kg;
    await saveState();
    renderWeightPanel();
    toast('Weight saved');
  });
  ...
  let _lastToday = todayKey();
  setInterval(() => {
    const today = todayKey();
    if (today !== _lastToday) {
      if (currentDate === _lastToday) currentDate = today;
      _lastToday = today;
      renderAll();
      return;
    }
    if (currentDate === today) renderAll();
  }, 60_000);
  ...
  const switchTab = (tab) => {
    ...
    if (tab === 'coach' && window.CoachAI) window.CoachAI.onTabOpen();
  };
  ...
  // Render cardio list (called after state loads so cal/min is accurate)
  ...
  (async () => {
        ...
        if (remote) {
          ...
          for (const [date, localDay] of Object.entries(local?.days || {})) {
            if (!merged.days[date]) {
              ...
              const weight = r.weight ?? l.weight;
              if (weight != null) merged.days[date].weight = weight;
            }
          }
          saved = merged;
    ...
    if (saved) {
      ...
      state.goals = { ...defaultState().goals, ...(saved.goals || {}) };
    }
