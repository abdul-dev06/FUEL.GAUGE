...
(() => {
  ...
  const drawSleepClock = (canvas, bedMin, wakeMin, goalHrs) => {
    ...
    // Sleep arc from bedtime to wake time
    // Map minutes-of-day → angle: 0 mins = -PI/2 (top), then clockwise
    const minToAngle = (m) => (m / (24 * 60)) * Math.PI * 2 - Math.PI / 2;
    const startA = minToAngle(bedMin);
    let endA = minToAngle(wakeMin);
    // If wake wraps past midnight, normalize so we draw the short arc forward
    if (wakeMin < bedMin || (wakeMin === bedMin)) endA += Math.PI * 2;
    if (endA <= startA) endA += Math.PI * 2;

    const dur = sleepDuration(bedMin, wakeMin) / 60;
    const meetsGoal = dur >= goalHrs * 0.875;  // within ~12.5% of goal
    const overshoot = dur > goalHrs * 1.15;     // overslept by 15%+
    ...
    // Overshoot indicator — outer cyan ring shows excess sleep over goal
    if (overshoot) {
      const overEnd = startA + Math.min((dur - goalHrs) / 24 * Math.PI * 2, Math.PI * 2);
      ctx.beginPath();
      ctx.arc(cx, cy, r + lineW * 1.7, startA, overEnd);
    ...
    // Bedtime moon dot
    const bedX = cx + Math.cos(startA) * r;
    const bedY = cy + Math.sin(startA) * r;
    ctx.beginPath();
    ctx.arc(bedX, bedY, lineW * 0.6, 0, Math.PI * 2);
    ctx.fillStyle = arcColor;
    ctx.fill();

    // Wake sun dot
    const wakeX = cx + Math.cos(endA) * r;
    const wakeY = cy + Math.sin(endA) * r;
    ctx.beginPath();
    ctx.arc(wakeX, wakeY, lineW * 0.6, 0, Math.PI * 2);
    ctx.fillStyle = arcColor;
    ctx.fill();
  };
  ...
  const buildHealthDataSummary = () => {
    ...
    return {
      ...
      stats: {
        ...
        avgSleepHrs: avg(sleptDays, 'hours'),
