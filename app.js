...
(() => {
  ...
  const renderStreak = () => {
    ...
    if (!dayHasEntry(todayKey)) d.setDate(d.getDate() - 1);
  ...
  const populateAppView = () => {
    const cur = document.documentElement.getAttribute('data-theme');
    document.querySelectorAll('#themePicker .theme-pick-btn').forEach(b => b.classList.toggle('active', b.dataset.val === cur));
    populateGroqSection();
    populateSyncSection();
  };

  const GROQ_KEY_STORAGE = 'fuelgauge.groq_key';

  const populateGroqSection = () => {
    let key = '';
    try { key = localStorage.getItem(GROQ_KEY_STORAGE) || ''; } catch {}
    const on = !!key;
    const badge = $('#groqBadge');
    badge.textContent = on ? 'ON' : 'OFF';
    badge.className = 'storage-badge' + (on ? ' badge-opfs' : '');
    $('#groqStatusLabel').textContent = on ? `Key saved (${key.slice(0, 8)}…)` : 'Not configured';
    $('#groqConfigLabel').textContent = on ? 'UPDATE KEY' : 'ADD KEY';
    if (on) $('#settingsGroqKey').value = key;
  };

  $('#groqConfigBtn').addEventListener('click', () => {
    const form = $('#groqCredsForm');
    const visible = form.style.display !== 'none';
    form.style.display = visible ? 'none' : 'flex';
    $('#groqChevron').style.transform = visible ? '' : 'rotate(180deg)';
  });

  $('#groqSaveBtn').addEventListener('click', () => {
    const val = $('#settingsGroqKey').value.trim();
    if (!val) { toast('Enter an API key.'); return; }
    try { localStorage.setItem(GROQ_KEY_STORAGE, val); } catch {}
    populateGroqSection();
    $('#groqCredsForm').style.display = 'none';
    $('#groqChevron').style.transform = '';
  ...
  (async () => {
    ...
    // Build exercises view (needs state.profile.weightKg for cal/min)
    initExercisesView();

    // Set initial tab state (ensures coachView is hidden on load)
    switchTab('home');
