...
(() => {
  ...
  const doLogin = async () => {
    ...
    try {
      ...
      const session = { id: user.id, jsonbin_key: atob(user.jsonbin_key).trim(), jsonbin_bin: atob(user.jsonbin_bin).trim(), groq_key: atob(user.groq_key).trim() };
  ...
  const openFormulaSheet = (title, html) => {
    $('#formulaSheetTitle').textContent = title;
    $('#formulaBody').innerHTML = html;
    openSheet('formulaSheet');
  };
  ...
  const formulaHtmlCalPerMin = () => {
    const met = currentExDetail?.met ?? null;
    if (!met) return '<div class="formula-note">Open an exercise to see specific values.</div>';
    const wkg = state.profile?.weightKg || 70;
    const cpm = calcCalPerMin(met);
    return `
      <div class="formula-section">
      ...
      <div class="formula-section">
        <div class="formula-section-title">Your Values</div>
        <div class="formula-eq">(${fHl('MET ' + met)} × ${fHl(wkg + ' kg')} × 3.5) ÷ 200</div>
      </div>
      <div class="formula-section">
        <div class="formula-section-title">Result</div>
        <div class="formula-result-wrap"><span class="formula-result">${cpm.toFixed(1)}</span><span class="formula-result-unit">cal / min</span></div>
      </div>
  ...
  // ── Formula ? button wiring ───────────────────────────────
  $('#bmrInfoBtn').addEventListener('click',       () => openFormulaSheet('BMR · MIFFLIN-ST JEOR',        formulaHtmlBMR()));
  $('#infoTDEEBtn').addEventListener('click',      () => openFormulaSheet('TDEE · TOTAL DAILY EXPENDITURE', formulaHtmlTDEE()));
  $('#goalsTDEEBtn').addEventListener('click',     () => openFormulaSheet('TDEE · TOTAL DAILY EXPENDITURE', formulaHtmlTDEE()));
  $('#balanceInfoBtn').addEventListener('click',   () => openFormulaSheet('ENERGY BALANCE',                formulaHtmlBalance()));
  $('#exCalInfoBtn').addEventListener('click',     () => openFormulaSheet('CAL / MIN · MET FORMULA',       formulaHtmlCalPerMin()));
  $('#autoMacrosInfoBtn').addEventListener('click',() => openFormulaSheet('AUTO-DISTRIBUTE MACROS',        formulaHtmlAutoMacros()));
  $('#goalPresetInfoBtn').addEventListener('click',() => openFormulaSheet('SET CALORIES FROM GOAL',        formulaHtmlGoalPreset()));
